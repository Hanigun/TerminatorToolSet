...
POST /api/warmup_start {root?}: прогон DDS->WebP по слоям ТОЛЬКО
открытого корня (basis + DLC). Саб-пути мода не греются: с открытым
модом прогон иначе цеплял бы всю игру. Пустой/битый root — не 400, а первый доступный
