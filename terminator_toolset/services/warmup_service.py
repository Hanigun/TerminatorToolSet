...
ТОЛЬКО открытого корня (basis + DLC). Саб-пути мода (ассеты/модели)
корнями прогрева НЕ являются: это пути поиска, а не слои — иначе
с открытым модом грелась бы заодно вся игра (саб-путь на корень
игры даёт её basis + DLC, ~10к файлов). Саб-пути греются
по требованию при первом просмотре через upr.dds_webp; свежий кэш
(не старше исходника) не трогается, повторный прогон — дешёвый
проход по mtime.
...
class WarmupManager:
    ...
    def _run(self, root):
        try:
            # СТРОГО открытый корень: саб-пути мода сюда не добавляем
            # (это пути поиска текстур, не корни прогрева).
            roots = [root]
            bases = []
            for r in roots:
