...
from . import spines as _spines

log = logging.getLogger("terminatorsheet.xml")
...
class Worksheet:
    ...
    def logical_row_count(self) -> int:
        """Позиция последней ЛОГИЧЕСКОЙ строки (учёт разреженных ss:Index у
        <Row>). Именно она должна стоять в ss:ExpandedRowCount: таблица из 16
        <Row> с ss:Index="36" у последней требует ExpandedRowCount=36."""
        pos = 0
        rows = ([self.header] if self.header is not None else []) + self.rows
        for r in rows:
            idx = self.row_index_attr(r)
            try:
    ...
    def _bump_row_count(self):
        self.table.set(_SS + "ExpandedRowCount", str(self.logical_row_count()))
...
def _decode(raw: bytes):
    """Return (text, encoding) for a spreadsheet bytes payload."""
...
class SpreadsheetML:
    """One parsed .xml spreadsheet file (live lxml tree + byte-splice source)."""

    def __init__(self):
        self.path: Optional[str] = None
        self.tree: Optional[etree._ElementTree] = None
        self.worksheets: list[Worksheet] = []
        self._spine: Optional[_spines.Spine] = None
        self._enc: str = "utf-8"
        self._text: str = ""
    ...
    def load(self, path: str, recover: bool = False) -> "SpreadsheetML":
        ...
        recover=False: строгое чтение, при синтаксической ошибке —
        SpreadsheetError (файл на диске никогда не меняется).
        recover=True: аварийный режим для физически битых XML (оборванные
        теги и т.п.): lxml восстанавливает то, что может, документ
        пересобирается (форматирование может измениться!), результат
        помечается recovered=True — сохранять только явно."""
        self.path = path
        self.recovered = bool(recover)
        with io.open(path, "rb") as fh:
            raw = fh.read()
        self._text, self._enc = _decode(raw)
        if not recover:
            # byte-splice engine: source of truth for the serialized bytes
            self._spine = _spines.Spine(self._text)
            ...
            try:
                ...
                raise SpreadsheetError(
                    "%s: это не валидный XML (структура не изменена): %s" % (
                        self.path, e)) from e
        else:
            self._load_recovered(raw)
        self._collect()
        return self

    def _load_recovered(self, raw: bytes):
        """Аварийное восстановление битого XML: lxml recover + re-serialize.
        Байт-точность теряется (это не byte-splice), но данные спасаются."""
        parser = etree.XMLParser(remove_blank_text=False, recover=True,
                                 resolve_entities=False)
        try:
            doc = etree.parse(io.BytesIO(raw), parser)
        except etree.XMLSyntaxError as e:
            raise SpreadsheetError("%s: восстановление не удалось: %s" % (
                self.path, e)) from e
        root = doc.getroot()
        if root is None:
            raise SpreadsheetError("%s: файл пуст или не читается" % self.path)
        ...
        for p in reversed(prevs):
            if not isinstance(p.tag, str):          # PI / comment
                parts.append(etree.tostring(p, encoding="unicode"))
                parts.append("\n")
        parts.append(etree.tostring(root, encoding="unicode"))
        ...
        parser = etree.XMLParser(remove_blank_text=False, recover=False,
                                 resolve_entities=False)
        try:
            self.tree = etree.fromstring(self._text.encode("utf-8"), parser)
        except etree.XMLSyntaxError as e:
            raise SpreadsheetError("%s: восстановленный файл не читается: %s" % (
                self.path, e)) from e
        self.tree = etree.ElementTree(self.tree)
        log.warning("recover-load: %s (doc salvaged, re-serialized)", self.path)

    def _parse_text(self):
        parser = etree.XMLParser(remove_blank_text=False, recover=False,
                                 resolve_entities=False)
        root = etree.fromstring(self._spine.text.encode(self._enc), parser)
        self.tree = etree.ElementTree(root)

    def from_string(self, data: str) -> "SpreadsheetML":
        self._text = data
        self._enc = "utf-8"
        self._spine = _spines.Spine(data)
        self._parse_text()
        self._collect()
        return self

    def _rebuild(self):
        """Re-derive the lxml model from the (byte-spliced) working text."""
        self._parse_text()
        self._collect()
    ...
    def to_bytes(self) -> bytes:
        # byte-preserving: the (edited) original text, not an lxml re-serialization.
        # Untouched parts stay byte-identical to the file on disk.
        if self._spine is None:
            raise SpreadsheetError("No document loaded")
        return self._spine.text.encode(self._enc)

    def to_string(self) -> str:
        return self.to_bytes().decode(self._enc)

    # -- byte-splice mutations (keep lxml + text in sync through the spine) ----
    def _spine_or_raise(self):
        if self._spine is None:
            raise SpreadsheetError("No document loaded")
        return self._spine

    def set_cell_value(self, sheet: int, row: int, col: int, value: str,
                       type_token: Optional[str] = None):
    ...
    def row_raw(self, sheet: int, row: int) -> str:
        ...
        ws = sp._ws(sheet)
        rec = ws["rows"][row]
        return sp.text[rec["start"]:rec["end"]]
    ...
    def fix_expanded_counts(self) -> list:
        ...
        «исправить файл»): возвращает список правок для UI."""
        sp = self._spine_or_raise()
        changed = _spines.fix_expanded_counts(sp)
        if changed:
            self._rebuild()
        return changed

    def fix_missing_styles(self) -> list:
        """Добавить определения <Style> для всех ss:StyleID, на которые
        ссылаются ячейки, но которых нет в <Styles> (Excel отказывается
        открывать такой файл). Возвращает список добавленных ID."""
        sp = self._spine_or_raise()
        added = _spines.fix_missing_styles(sp)
        if added:
            self._rebuild()
        return added
    ...
    def save(self, path: Optional[str] = None, validate: bool = True):
        ...
        data = self.to_bytes()
        if validate:
            self._validate(data.decode(self._enc))
        with io.open(target, "wb") as fh:
            fh.write(data)

    def _validate(self, text: str):
        try:
            etree.fromstring(text.encode(self._enc))
        except etree.XMLSyntaxError as e:
        ...
        # Счётчики ss:ExpandedRowCount/ColumnCount, не совпадающие с фактом,
        # НЕ блокируют запись (вход = выход), но фиксируются в логе: именно
        # из-за них Excel отказывается открывать файл. Лечится кнопкой
        # «исправить файл» (/api/fix_file).
        for ws2 in s2.worksheets:
            fact_rows = ws2.logical_row_count()
            declared_rows = ws2.expanded_row_count()
            if declared_rows and declared_rows != fact_rows:
                log.warning("save %s: sheet %r ExpandedRowCount=%d, fact=%d "
                            "(Excel может не открыть; /api/fix_file лечит)",
                            self.path, ws2.name, declared_rows, fact_rows)
            fact_cols = ws2.column_count()
            declared_cols = ws2.expanded_column_count()
            if declared_cols and declared_cols < fact_cols:
                log.warning("save %s: sheet %r ExpandedColumnCount=%d < fact=%d "
                            "(Excel может не открыть; /api/fix_file лечит)",
                            self.path, ws2.name, declared_cols, fact_cols)
