...
// ---------- G10: self-updates (worker over GitHub releases) ----------
{
  ...
  const bad = section("UPD GATE PASSED", [
    ...
    [/\/api\/update_restart/.test(apiupd) && /def restart\(self\)/.test(updsvc) && /_write_updater/.test(updsvc) && /robocopy/.test(updsvc), "backend must restart via an external updater (robocopy after our death, never self-replace while locked)"],
